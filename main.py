...
from forms import CommentForm, LoginForm, RegisterForm, CreatePostForm
from flask_gravatar import Gravatar
import os
...
# check day69 last solutions comment to upgrade comment area with date, delete btn etc.
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            flash("The email address doesnt exists!")
            return redirect(url_for("login"))
...
# _ Use a decorator so only an admin user can create a new post
